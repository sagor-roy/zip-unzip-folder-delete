...
if (isset($_POST['dodelete'])) {
  $selectedFolder = $_POST['folderdelete'];

  if (!empty($selectedFolder)) {
    $folderToDelete = $directory . '/' . $selectedFolder;

    if (is_dir($folderToDelete)) {
      // Delete the selected folder and its contents
      deleteFolder($folderToDelete);
      //echo "Folder '$selectedFolder' deleted successfully!";
      $GLOBALS['status'] = array("success" => "Folder $selectedFolder deleted successfully!");
    } else {
     // echo "Folder '$selectedFolder' not found.";
       $GLOBALS['status'] = array("error" => "Folder $selectedFolder not found!");
    }
  }
}
...
<form action="" method="POST">
  <fieldset>
   ...
   <fieldset>
      <h1>Folder Delete</h1>
      <label for="zipfile">Select your folder want to delete:</label>
      <select name="folderdelete" size="1" class="select">
        <?php foreach ($folders as $folder) {
          echo "<option value=\"$folder\">$folder</option>";
        } ?>
      </select>
      <input type="submit" name="dodelete" class="submit" value="Confirm Delete" />
    </fieldset>
